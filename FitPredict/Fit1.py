'''探究品类销量与销售单价的关系'''
# acutally we need raletionship between sales and price for each day!! 
# not the cumulated sales and price for each month!!
# since for month, then set the mutiply factor the lower and upper bound for each month
# so test the outline factor for each month
# it seems that 0.5 and 1.5 are suitable for the lower and upper bound
...
file_path ='按月分组品类销量和单价.xlsx'
Combine = pd.read_excel(file_path)
print(Combine)
